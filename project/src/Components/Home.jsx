...
export default function Home() {
  ...
  // بيانات وهمية للإحصائيات (مطابقة للصورة)
  const stats = [
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto font-sans text-gray-800">
      ...
      <div className="bg-white rounded-3xl p-8 md:p-12 shadow-sm border border-gray-100 flex flex-col md:flex-row items-center gap-10 mb-8">
        ...
        <div className="w-full md:w-1/2 space-y-6">
          ...
          <div className="flex flex-wrap gap-4 pt-2">
            <Link to="/tournaments" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-lg shadow-lg shadow-blue-200 transition-all">
              BROWSE TOURNAMENTS
            </Link>
            <Link to="/login" className="bg-white hover:bg-gray-50 text-gray-700 border border-gray-300 font-bold py-3 px-8 rounded-lg transition-all">
              SIGN IN
            </Link>
          </div>
        </div>
        
        {/* صورة الهيرو (Placeholder) */}
        <div className="w-full md:w-1/2 h-80 bg-gray-100 rounded-2xl flex items-center justify-center border-2 border-dashed border-gray-200">
            <span className="text-gray-400 font-bold text-lg">HERO IMAGE</span>
        </div>
      </div>

      {/* --- 2. Search Bar --- */}
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 mb-10 items-center">
        ...
        <button className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-lg transition-colors">
            SEARCH
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
        {stats.map((stat) => (
            <div key={stat.id} className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center hover:shadow-md transition-shadow">
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-6">
            ...
            <Link to="/tournaments" className="bg-white border border-gray-300 hover:bg-gray-50 text-gray-600 text-sm font-bold py-2 px-6 rounded-lg transition-colors">
                VIEW ALL
            </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuredTournaments.map((tournament) => (
                <div key={tournament.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg transition-all overflow-hidden flex flex-col p-5">
                    {/* صورة */}
                    ...
                    <div className="flex justify-between items-center mt-auto border-t border-gray-100 pt-4">
                        ...
                        <button className="flex items-center text-blue-600 hover:text-blue-800 text-xs font-bold uppercase">
